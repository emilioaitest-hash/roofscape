...
**Decided.** v1 confines agents to their building's workspace by path, runs shell
commands under an allowlist, and escalates anything unrecognised. Code work
happens in a dedicated git worktree. A container per building arrives before
public release.
...
The interface is written so the container is a swap behind it.
